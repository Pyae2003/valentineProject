...
import { profileSetupSchema } from "../schema";
import { createProfileImageUploadUrls } from "../actions/profileImageUploadUrl";
import { createProfileImage } from "../actions/add-profileImage";

const SetUpPage = () => {
  const { execute, isPending, hasErrored, hasSucceeded } =
    useAction(createProfileImage);
  ...
  async function onSubmit({
    ...
    girlName,
    girlImage,
  }: z.infer<typeof profileSetupSchema>) {
    if (!boyImage || !girlImage) {
      toast.warning("Please select all image 📸", {
        position: "top-center",
      });
      return;
    }
